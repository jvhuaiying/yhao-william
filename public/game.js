...
class PlayScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.physics.add.collider(this.hero0, this.platforms);
    ...
    update() {
        this.cameras.main.setBounds(0, -this.hero0.yChange, this.cameras.main.width, this.cameras.main.height + this.hero0.yChange);


        // 根据更高的角色移动camera
        const higherHeroY = Math.min(this.hero0.y, this.hero1.y);

        if (typeof this.maxHeightReached === 'undefined' || higherHeroY < this.maxHeightReached) {
            this.maxHeightReached = higherHeroY;
        }
        
        this.hero0.yChange = Math.max(this.hero0.yChange, Math.abs(this.hero0.y - this.hero0.yOrig));
        this.hero1.yChange = Math.max(this.hero1.yChange, Math.abs(this.hero1.y - this.hero1.yOrig));
        
        const higherYChange = Math.max(this.hero0.yChange, this.hero1.yChange);
        ...
        if (keys.A.isDown) {
            this.hero1.body.setVelocityX(-250);
        } else if (keys.D.isDown) {
            this.hero1.body.setVelocityX(250);
        } else {
            this.hero1.body.setVelocityX(0);
        }
        
        if (keys.W.isDown && this.hero1.body.touching.down) {
            this.hero1.body.setVelocityY(-400);
        }

        if (this.cursor.left.isDown) {
            this.hero0.body.setVelocityX(-250);
            this.hero0.anims.play('left-walk', true);
            this.lastDirection = 'left'; // 注意这里的变化
        } else if (this.cursor.right.isDown) {
            this.hero0.body.setVelocityX(250);
            this.hero0.anims.play('right-walk', true);
            this.lastDirection = 'right'; // 注意这里的变化
        } else {
            this.hero0.body.setVelocityX(0);
            if (this.lastDirection === 'left') { // 注意这里的变化
                this.hero0.anims.play('left-stand');
            } else {
                this.hero0.anims.play('right-stand');
            }
        }
        

        if (this.cursor.up.isDown && this.hero0.body.touching.down) {
            this.hero0.body.setVelocityY(-400);
        }

        this.physics.world.wrap(this.hero0, 20);
        ...
        this.platforms.getChildren().forEach(platform => {
            if (this.hero0.body.touching.down && platform.getBounds().contains(this.hero0.x, this.hero0.y - 1)) {
                this.lastPlatformY = platform.y;
            }
        });
    
        // 检查任意一个角色是否已经掉落出屏幕下方
        if (this.hero0.y > this.cameras.main.scrollY + this.cameras.main.height+110 || 
            this.hero1.y > this.cameras.main.scrollY + this.cameras.main.height+110) {
            this.bgm.stop();
            this.scene.restart();
        }
        ...
        // 播放碰撞音效
        if (this.hero0.body.touching.down && !this.heroWasTouchingDown) {
            this.collisionSound.play();
        }
        this.heroWasTouchingDown = this.hero0.body.touching.down;
    ...
    heroCreate() {
        this.hero0 = this.add.sprite(this.cameras.main.centerX, this.cameras.main.height - 36, 'hero');
        this.hero0.setOrigin(0.5);
        this.hero0.setScale(0.13);

        this.hero0.yOrig = this.hero0.y;
        this.hero0.yChange = 0;

        this.physics.add.existing(this.hero0);
        this.hero0.body.setGravityY(500);
        this.hero0.body.checkCollision.up = false;
        this.hero0.body.checkCollision.left = true;
        this.hero0.body.checkCollision.right = true;

        // 第二个角色
        this.hero1 = this.add.sprite(this.cameras.main.centerX, this.cameras.main.height - 36, 'hero');
        this.hero1.setOrigin(0.5);
        this.hero1.setScale(0.13);
        this.hero1.yOrig = this.hero1.y;
        this.hero1.yChange = 0;
        this.physics.add.existing(this.hero1);
        this.hero1.body.setGravityY(500);
        this.hero1.setDepth(100);
        this.hero1.body.checkCollision.up = false;
        this.hero1.body.checkCollision.left = true;
        this.hero1.body.checkCollision.right = true;

        // 添加碰撞检测
        this.physics.add.collider(this.hero0, this.platforms);
        this.physics.add.collider(this.hero1, this.platforms);
        // 添加两个角色之间的碰撞检测
        this.physics.add.collider(this.hero0, this.hero1);
    }
}
